    ...
    private void loadAndPrintText(Identifier path) {
        try (BufferedReader reader = new BufferedReader(this.minecraft.getResourceManager().openAsReader(path))) {
            ...
            if (e instanceof FileNotFoundException) {
                CommonMain.LOGGER.warn("No text file found for " + this.getClass().getSimpleName() + ": " + path);
            } else {
                e.printStackTrace();
            }
        }
    ...
    private LineObject parseLine(String line) {
        ...
        if (line.startsWith("!image:")) {
            ...
            for (int i = 1; i < parts.length; i++) {
                if (parts[i].startsWith("scale=")) {
                    try {
                        ...
                        CommonMain.LOGGER.error("Invalid scale format in line: " + line);
                    }
                }
            }
            ...
            try {
                ...
                if (e instanceof FileNotFoundException) {
                    CommonMain.LOGGER.error("No image file found in referencing text file " + this.getClass().getSimpleName() + ": " + imageId);
                } else {
                    e.printStackTrace();
                }
