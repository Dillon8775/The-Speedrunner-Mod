...
public class SpeedrunnerShieldDecorationRecipe extends CustomRecipe {
    public static final MapCodec<SpeedrunnerShieldDecorationRecipe> MAP_CODEC = RecordCodecBuilder.mapCodec(
            i -> i.group(
                            Ingredient.CODEC.fieldOf("banner").forGetter(o -> o.banner),
                            Ingredient.CODEC.fieldOf("target").forGetter(o -> o.target),
                            ItemStackTemplate.CODEC.fieldOf("result").forGetter(o -> o.result)
                    )
                    .apply(i, SpeedrunnerShieldDecorationRecipe::new)
    );
    public static final StreamCodec<RegistryFriendlyByteBuf, SpeedrunnerShieldDecorationRecipe> STREAM_CODEC = StreamCodec.composite(
            ...
            o -> o.result,
            SpeedrunnerShieldDecorationRecipe::new
    );
    public static final RecipeSerializer<SpeedrunnerShieldDecorationRecipe> SERIALIZER = new RecipeSerializer<>(MAP_CODEC, STREAM_CODEC);
    private final Ingredient banner;
    private final Ingredient target;
    private final ItemStackTemplate result;

    public SpeedrunnerShieldDecorationRecipe(final Ingredient banner, final Ingredient target, final ItemStackTemplate result) {
        this.banner = banner;
        this.target = target;
        this.result = result;
    }
