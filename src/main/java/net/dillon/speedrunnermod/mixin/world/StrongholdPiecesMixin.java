...
public class StrongholdPiecesMixin {
    ...
    static {
        if (common().accessibility().modifiedStrongholdGeneration && !isBalancedMode()) {
            if (isDoomMode()) {
                int libraries = common().structureConfigs().stronghold.totalLibrariesPerWorld();
                if (libraries > 4) {
                    libraries = 4;
                }
                STRONGHOLD_PIECE_WEIGHTS = new StrongholdPieces.PieceWeight[]{
                        ...
                        new StrongholdPieces.PieceWeight(StrongholdPieces.Library.class, 100, libraries * 2) {
