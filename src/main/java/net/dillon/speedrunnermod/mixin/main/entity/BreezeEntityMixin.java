...
public class BreezeEntityMixin {
    ...
    public static DefaultAttributeContainer.Builder createBreezeAttributes() {
        final double genericMovementSpeed = isPlayingModeDoom() ? 0.65D : 0.50D;
        final double genericMaxHealth = isPlayingModeDoom() ? 35.0D : 25.0D;
        final double genericFollowRange = isPlayingModeDoom() ? 32.0D : 16.0D;
        final double genericAttackDamage = isPlayingModeDoom() ? 3.5D : 2.0D;
