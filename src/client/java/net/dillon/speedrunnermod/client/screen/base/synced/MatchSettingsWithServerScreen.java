...
public class MatchSettingsWithServerScreen extends AbstractModScreen {
    ...
    public void renderCustomText(GuiGraphicsExtractor context) {
        if (this.matchAndRestartButton.active) {
            context.centeredText(this.font, ModTexts.MATCH_SETTINGS_WITH_SERVER_LINE1, this.width / 2, 110, CommonColors.WHITE);
            context.centeredText(this.font, ModTexts.MATCH_SETTINGS_WITH_SERVER_LINE2, this.width / 2, 130, CommonColors.WHITE);
        } else {
            context.centeredText(this.font, ModTexts.MATCH_SETTINGS_WITH_SERVER_SYNC_FAILED, this.width / 2, 110, CommonColors.WHITE);
            context.centeredText(this.font, ModTexts.MATCH_SETTINGS_WITH_SERVER_SYNC_FAILED_LINE2, this.width / 2, 130, CommonColors.WHITE);
        }
    }
